  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      if (!existsSync(distDir)) {
        throw new Error(
          "Plugin dist/ directory not found. Run 'bun run build' first.",
        )
      }
    ...
    describe("without plugin", () => {
      ...
      test("very large image should fail with size error", async () => {
        const result = await runOpencode(
          configWithoutPlugin,
          join(fixturesDir, "very-large.jpg"),
        )
        expect(
          hasImageSizeError(result.stderr) ||
            hasImageSizeError(result.stdout) ||
    ...
    describe("edge cases", () => {
      test("should handle non-existent image gracefully", async () => {
        const result = await runOpencode(
          configWithPlugin,
          join(fixturesDir, "non-existent.jpg"),
        )
      ...
      test("should handle text-only prompts without images", async () => {
        ...
        try {
          ...
          expect(hasImageSizeError(error.stderr || error.message || "")).toBe(
            false,
          )
